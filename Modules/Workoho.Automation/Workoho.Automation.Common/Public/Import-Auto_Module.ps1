...
.EXAMPLE
    PS> Import-Auto_Module -Name @('Module1', 'Module2')

    Imports 'Module1' and 'Module2' silently.
#>

function Import-Auto_Module {
    [CmdletBinding()]
    Param(
    ...
    @($Name) | ForEach-Object {
        ...
        try {
            Write-Debug "[Import-Auto_Module]: - Silently importing module $($Module.Name)"
            ...
            if ($IsOptional) {
                Write-Warning "[Import-Auto_Module]: - Optional module could not be loaded: $(Module.Name)"
            }
            else {
                $Module.Remove('Debug')
