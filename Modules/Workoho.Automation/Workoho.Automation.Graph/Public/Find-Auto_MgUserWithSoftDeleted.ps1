...
.PARAMETER Property
    ...
    You may use the Confirm-Auto_MgDirectoryRoleActiveAssignment function in your runbook to check if the role is assigned.
...
function Find-Auto_MgUserWithSoftDeleted {
    [CmdletBinding()]
    Param(
    ...
    @($UserId) | ForEach-Object { ($_ -replace '\s', '').Split(',') } | ForEach-Object {
        ...
        try {
            $response = Invoke-Auto_MgGraphRequest $params
        }
        catch {
            Throw $_
        }
