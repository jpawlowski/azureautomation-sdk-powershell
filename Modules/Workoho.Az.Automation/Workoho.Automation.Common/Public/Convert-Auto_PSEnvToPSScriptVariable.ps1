...
.DESCRIPTION
    ...
    When used in conjunction with the Import-Auto_AzAutomationVariableToPSEnv function, it allows for the import of Azure Automation variables to PowerShell environment variables and then to script variables.
...
.EXAMPLE
    PS> Convert-Auto_PSEnvToPSScriptVariable -Variable MyVariable -ScriptParameterOnly $true

    This example converts the environment variable 'MyVariable' to a script variable, respecting script parameters only.
#>

function Convert-Auto_PSEnvToPSScriptVariable {
    [CmdletBinding()]
    Param(
