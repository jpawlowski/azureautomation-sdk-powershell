...
.EXAMPLE
    PS> Write-Auto_CsvOutput -InputObject $data
    This example converts the $data object to CSV, and writes it to the output stream.

.EXAMPLE
    PS> Write-Auto_CsvOutput -InputObject $data -StorageUri 'https://mystorageaccount.blob.core.windows.net/mycontainer/myblob.csv?sastoken'
    This example converts the $data object to CSV, and uploads it to the specified blob storage.

.EXAMPLE
    PS> Write-Auto_CsvOutput -InputObject $data -StorageUri 'https://mystorageaccount.file.core.windows.net/myshare/myfile.csv?sastoken'
    This example converts the $data object to CSV, and uploads it to the specified file storage.
#>

function Write-Auto_CsvOutput {
    [CmdletBinding()]
    Param(
        ...
        process {
            ...
            function Convert-Value {
                ...
                elseif ($Value -is [System.Collections.IEnumerable] -and -not ($Value -is [string])) {
                    ...
                    return Convert-Array -Array $Value -CurrentDepth ($CurrentDepth + 1)
                }
                elseif ($Value -is [System.Collections.Hashtable]) {
                    Write-Debug "Converting Hashtable value"
                    Convert-Hashtable -Hashtable $Value -CurrentDepth ($CurrentDepth + 1)
            ...
            function Convert-Object {
                ...
                if ($Obj -is [System.Collections.IDictionary]) {
                    Convert-Hashtable -Hashtable $Obj -CurrentDepth $CurrentDepth
                ...
                elseif ($Obj -is [System.Collections.IEnumerable] -and -not ($Obj -is [string])) {
                    foreach ($item in $Obj) {
                        Convert-Object -Obj $item -CurrentDepth ($CurrentDepth + 1)
                    }
                }
            ...
            if ($InputObject -is [System.Collections.IEnumerable] -and -not ($InputObject -is [string])) {
                ...
                foreach ($item in $InputObject) {
                    if ($item -is [PSObject] -or $item -is [System.Collections.IDictionary]) {
                        $processedItems.Add((Convert-Object -Obj $item -CurrentDepth 0))
                    }
                    else {
            ...
            elseif ($InputObject -is [PSObject] -or $InputObject -is [System.Collections.IDictionary]) {
                $InputObject = Convert-Object -Obj $InputObject -CurrentDepth 0
            }
