...
.EXAMPLE
    PS> Wait-AzA_AzAutomationConcurrentJob

    Waits for concurrent jobs in Azure Automation.
#>

function Wait-AzA_AzAutomationConcurrentJob {
    [CmdletBinding()]
    Param()
    ...
    if ($IsAzureAutomationJob) {
        ...
        # Implicitly connect to Azure Graph API using the Connect-AzA_MgGraph command.
        ...
        if ($env:AZURE_AUTOMATION_ResourceGroupName -and $env:AZURE_AUTOMATION_AccountName -and $env:AZURE_AUTOMATION_RUNBOOK_Name) {
            ...
            do {
                ...
                (Invoke-AzA_AzRestMethod $params).Content.value.properties |
                    & {
                        process {
                            if (
