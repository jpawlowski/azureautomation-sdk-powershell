...
.EXAMPLE
    PS> Convert-AzA_UserIdToLocalUserId -UserId 'john.doe@contoso.com', 'jane.doe@example.com'

    This example demonstrates how to convert an array of user IDs to local UPNs using the tenant's default verified domain.
#>

function Convert-AzA_UserIdToLocalUserId {
    [CmdletBinding()]
    Param(
    ...
    $tenantVerifiedDomains = if ($VerifiedDomains) { $VerifiedDomains } else {
        ...
        Connect-AzA_MgGraph -Scopes @(
            'Organization.Read.All'
        )
        #endregion ---------------------------------------------------------------------

        try {
        (Invoke-AzA_MgGraphRequest @{ Uri = '/v1.0/organization'; ErrorAction = 'Stop'; Verbose = $false; Debug = $false }).Value.VerifiedDomains
        }
        catch {
            Throw $_
        }
