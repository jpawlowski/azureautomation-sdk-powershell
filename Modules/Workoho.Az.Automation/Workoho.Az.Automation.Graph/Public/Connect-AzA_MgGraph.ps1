...
.EXAMPLE
    PS> Connect-AzA_MgGraph -Scopes @('User.Read', 'Mail.Read') -TenantId 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx'

    Connects to Microsoft Graph using the specified scopes and the specified tenant ID.
#>

function Connect-AzA_MgGraph {
    ...
    function Get-MgMissingScope ([Array]$Scopes) {
        ...
        foreach ($Scope in $Scopes) {
            if ($WhatIfPreference -and ($Scope -like '*Write*')) {
                Write-Verbose "[Connect-AzA_MgGraph]: - What If: Removed $Scope from required Microsoft Graph scopes"
    ...
    if ($TenantId) {
        if (
            ...
            Throw '[Connect-AzA_MgGraph]: - Invalid tenant ID. The tenant ID must be a valid GUID.'
        }
    ...
    if (
        ...
        if ($IsAzureAutomationJob) {
            Write-Verbose '[Connect-AzA_MgGraph]: - Using system-assigned Managed Service Identity'
        ...
        elseif (
            ...
            Write-Verbose '[Connect-AzA_MgGraph]: - Using device code authentication'
            $params.UseDeviceCode = $true
            if ($Scopes) { $params.Scopes = $Scopes }
        }
        else {
            Write-Verbose '[Connect-AzA_MgGraph]: - Using interactive sign in'
            if ($Scopes) { $params.Scopes = $Scopes }
        }
    ...
    if (
        ...
        try {
            ...
            if ($Context.AuthType -eq 'Delegated') {
                ...
                Write-Verbose "[Connect-AzA_MgGraph]: - Getting user details for $($Context.Account) ..."
                ...
                Write-Verbose "[Connect-AzA_MgGraph]: - Getting service principal details for $($Context.ClientId) ..."
            ...
            Write-Verbose "[Connect-AzA_MgGraph]: - Setting environment MG_PRINCIPAL_ID to '$($Principal.Id)' and MG_PRINCIPAL_DISPLAYNAME to '$($Principal.DisplayName)' ..."
...
New-Alias -Name 'Connect-AzA_Graph' -Value 'Connect-AzA_MgGraph' -Force
$ModuleMemberExport.Alias.Add('Connect-AzA_Graph')
